...
AI Network를 이용하여 입력 영상의 특성에 따라 적응적으로 대응하는 영상 코덱의 전/후처리 방법을 제공한다. 

구체적으로는 시간 및 공간 상에서의 매우 다양하게 변화하는 영상 신호의 특징을 충분히 반영할 수 있도록 시/공간 해상도 변환을 위한 전/후처리 모듈을 다양한 조합으로 구성하고, 특정 입력 영상의 시/공간 특성 및 비디오 코덱이 운용되는 상황을 고려하여 영상 왜곡 및 압축률에 대한 제한 조건을 부여하고, 이를 통해 최적의 전/후처리 모듈 조합을 선택하는 방법이다. 또한, 전처리 모듈을 구현함에 있어서 기존 방법과는 달리 인공 신경망 기반 학습 기술을 통해 시/공간축으로의 해상도 및 영상 신호를 변형함으로써 전체적인 코덱 압축률을 보다 효과적으로 높일 수 있는 장치 및 방법을 제공한다.
...
그림3에서 전처리 및 후처리 네트워크는 복원 영상과 원본 영상의 Distortion을 손실함수로 정의하여 오차 역전파를 통해 학습된다. 이때 레거시 코덱은 미분 불가능한 양자화 연산을 가지고 있기 때문에 그라디언트의 전달이 불가능하므로 End-to-end 학습이 불가능하다. 이를 해결하기 위해 기존 레거시 코덱을 모델링하여 생성한 코덱 모사 네트워크를 사용하여 기존 코덱 대신 그라디언트의 전달 통로로 사용하여 전처리 및 후처리 네트워크를 동시에 최적화 한다. 
